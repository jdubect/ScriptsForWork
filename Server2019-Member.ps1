...
# Sets Windows update to 'notify only'.
...
# Renames Local Guest Account to 'TAB'
Write-Host "Renaming Local Guest Account" -ForegroundColor Green
timeout /t -1
Rename-LocalUser -Name "guest" -NewName "TAB"

# Sets 'TAB' password
Write-Host "Set password for TAB Account" -ForegroundColor Green
$Password = Read-Host "Enter a new Password" -AsSecureString
$UserAccount = Get-LocalUser -Name "TAB"
